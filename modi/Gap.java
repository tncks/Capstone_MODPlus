...
public class Gap implements SpecInterpretation {
    ...
    public double getBStartMass() {
        return bStartMass;
    }

    public double getYStartMass() {
        return yStartMass;
    }

    public Peptide getMatchedPeptide() {
        return matchedPeptide;
    }

    public Sequence sequence() {
        return matchedPeptide.subSequence(start, end + 1);
    }
    ...
    public PTMPosition getPosition() {
        return position;
    }

    public GapInterpretation getGapInterpretation() {
        return gapInterpretation;
    }
    ...
    public boolean isInModifiedRange(double v, ScanContext__ context) {
        if (context.getMinModifiedMass() - context.getGapTolerance() < v && v < context.getMaxModifiedMass() + context.getGapTolerance())
            return true;
        else return Math.abs(v) <= context.getGapTolerance();
    }
    ...
    public boolean isReasonable(ScanContext__ context) {
        if (!isInModifiedRange(this.offset, context)) return false;
        ...
        // very changed (important two line)
        PTMDB myVariableModifications = context.getVariableModifications();
        double minMod = myVariableModifications.minimumModifiedMass(seq, pos);
        double maxMod = myVariableModifications.maximumModifiedMass(seq, pos);
